...
ArchipelGenerator::~ArchipelGenerator() {}

  int ArchipelGenerator::getNbBloc(int nb) const {
  	int random = rand() % 100;
  if (random < 56) {
    return -1;
  } else {
  ...
  double ArchipelGenerator::getProbaAir(int height, int maxHeight) const {
	return std::max((double(height) / maxHeight) - 0.5 * maxHeight, 0.0);
}

  double ArchipelGenerator::getProbaAdminium(int height, int maxHeight) const {
  ...
  double ArchipelGenerator::getProbaCactus(int height, int maxHeight) const {
	double u = double(height) / maxHeight;
	if (u > 0.9) {
		return 0.005;
	} else {
  ...
  double ArchipelGenerator::getProbaCoal(int height, int maxHeight) const {
  ...
  double ArchipelGenerator::getProbaDirt(int height, int maxHeight) const {
	double u = double(height) / maxHeight;
	if (u > 0.7) {
		return 0.2;
	} else {
  ...
  double ArchipelGenerator::getProbaFolliage(int height, int maxHeight) const {
	return 0.0;
}

  double ArchipelGenerator::getProbaGlowstone(int height, int maxHeight) const {
	return 0.0;
}

  double ArchipelGenerator::getProbaGold(int height, int maxHeight) const {
	return getProbaCoal(height * 0.5, maxHeight) * 0.1;
}

  double ArchipelGenerator::getProbaDiamond(int height, int maxHeight) const {
	return getProbaGold(height * 0.7, maxHeight);
}

  double ArchipelGenerator::getProbaGrass(int height, int maxHeight) const {
		return 0.0;
	
}

  double ArchipelGenerator::getProbaIron(int height, int maxHeight) const {
	return getProbaCoal(height, maxHeight) * 0.5;
}

  double ArchipelGenerator::getProbaSand(int height, int maxHeight) const {
	double u = double(height) / maxHeight;
	if (height == maxHeight) {
		return 0.9;
	} else if (u > 0.8) {
  ...
  double ArchipelGenerator::getProbaSnow(int height, int maxHeight) const {
	return 0;
}

  double ArchipelGenerator::getProbaSnowBlock(int height, int maxHeight) const {
	return 0;
}

  double ArchipelGenerator::getProbaStone(int height, int maxHeight) const {
	double u = double(height) / maxHeight;
	if (u > 0.9) {
		return 0.01;
	} else {
  ...
  double ArchipelGenerator::getProbaWood(int height, int maxHeight) const {
	return 0;
}

 std::vector<double> ArchipelGenerator::getProbaBloc(int depth, int maxHeight, int superMaxHeight) const {
	    	...
	    	for (int x = 1; x < (wSize - 1); ++x ) {
	    		if (map(x, y, z, MapChannels::BLOC) == CubeTypes::WATER) {
	    			for (int w = 0; w < 9; ++w) {
	    				...
	    				if ((map(x - 1 + (w / 3), y,  z - 1 + (w %3), MapChannels::BLOC) == CubeTypes::AIR) && ((rand() % 100 < 80))) {
